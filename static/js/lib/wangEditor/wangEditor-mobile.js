...
window.___E_mod(function(E, $) {
	...
	E.config = {
		...
		// 七牛Token
		uploadToken: sessionStorage.getItem('qiniuToken'),
		...
		menus[menuId] = {
			...
			bindEvent: function(editor) {
				...
				$inputFlie.on('change', function(e) {
					...
					if(uploadImgUrl === '' || uploadToken === '') {
						// 没有配置七牛域名 或七牛Token
						console.log('没有配置 uploadImgUrl或uploadToken ，wangEditor 将无法上传图片。');
						return;
					}
...
//						console.log('生成预览图片，src是：' + prevImgSrc);
						...
						function timeoutCallback() {
							console.log('访问超时（配置的超时事件是：' + timeout + '）');
...
//							}
				        ...
				        var imgname = file.name.slice(0,file.name.lastIndexOf('.'));
				        var suffix = file.name.slice(file.name.lastIndexOf('.') + 1);
				        var imageSrc = imgname+'_'+new Date().getTime()+'.'+suffix;
				        
				        formData.append('key', imageSrc);  
						...
						xhr.onreadystatechange = function() {
							...
							if (xhr.readyState == 4 && xhr.status == 200&& xhr.responseText != "") {  
								...
								console.log('上传成功');
				                var resultSrc = uploadImgUrl +'/' +JSON.parse(xhr.responseText).key; //服务器端要返回图片url地址
								var erroInfo;
								var $prevImgContainer = $('#' + prveImgContainerId);
								var loadImg;
								var $loadImg;
...
//									console.log('下载完成，正式呈现在编辑区域');
								...
								loadImg.onerror = function() {
									console.log('图片加载失败，请确定这个url是否能成功得到图片：' + resultSrc);
